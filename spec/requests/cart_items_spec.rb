...
RSpec.describe 'Cart items API', type: :request do
  ...
  describe 'GET /cart_items' do
    ...
    context 'then authorized' do
      ...
      it 'can add product into cart' do
        post '/cart_items', headers: auth_params, params: { product_id: products[2].id }
        expect(parsed_response_data.map { |d| d['id'].to_i }).to eq([products[0].id, products[1].id, products[2].id])
      end

      it 'can delete products from cart' do
        delete "/cart_items/#{products[1].id}", headers: auth_params
        expect(parsed_response_data.map { |d| d['id'].to_i }).to eq([products[0].id])
      end
    end
  end
end
